...
*/
function testListOut($utype)
{
  $USER_TYPE = 1;            //模块用户权限
  $ListTitle = "核算检测记录";     //模块标题
  $psize = 10;                 //每页记录数量
  ...
  $res = GetTestList($utype,$page,$psize,$sea);
            ...
            <nav aria-label="Page navigation">
              <ul class="pagination">
                <li '.( ($page==1) ? 'class="disabled"' : '' ).'>
                  <'.( ($page==1) ? 'span' : 'a' ).' href="system.php?list=test&sea='.$sea.'&page='.($page-1).'" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                  </'.( ($page==1) ? 'span' : 'a' ).'>
                </li>
                ';
                for($i=1;$i<=$pnum;++$i) echo '<li><a href="system.php?list=test&sea='.$sea.'&page='.$i.'">'.$i.'</a></li>';
  echo '
                <li '.( ($page>=$pnum) ? 'class="disabled"' : '' ).'>
                  <'.( ($page>=$pnum) ? 'span' : 'a' ).' href="system.php?list=test&sea='.$sea.'&page='.($page+1).'" aria-label="Next">
